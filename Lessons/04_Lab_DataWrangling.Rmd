...
TuesdayLake_90_99 <- NTL.phys.data %>%
...
> Answer: Row numbers arent as reliable.
...
NTL.phys.data.month <- mutate(NTL.phys.data, month = month(sampledate))

TuesdayLake_90_99_July <- NTL.phys.data %>%
...
TuesdayLake_90_99_July_MST <-  TuesdayLake_90_99_July %>%
...
pivotingLonger <- pivot_longer(NTL.phys.data, cols = irradianceWater:irradianceDeck, names_to = "location", values_to = "irradiance_depth")


#Exercise 6: spread temperatureC into more than one column based on the depth

pivotingWider <- pivot_wider(NTL.phys.data, names_from = depth, values_from = temperature_C)
