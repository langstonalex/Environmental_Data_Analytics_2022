...
- **Why R Markdown**  
  ...
  * *Variety of languages can be run*
  * *Managing how code is displayed*
  * *Can add formatted documentation*
  * *Reproducability*
...
------------- | -------------
EPA Air | EPA's dataset containing measurements of air pollutants
North Temperate Lakes | Measurements on physical makeup of a range of lakes in Wisconsin
Niwot Ridge NEON | Data on the woody debris and litter at sites in the Southern Rockies
NWIS Site Flow | Location and readings of stream flow gauges  
...
setwd("C:/Users/lwa8/Documents/R/ENV872/Environmental_Data_Analytics_2022")
getwd()
...
processed_lake_nutrients <- Lake_nutrients %>%
...
summary_tn <- summarise(processed_lake_nutrients, 
                        tn_mean = mean(tn_ug), 
                        tn_min = min(tn_ug), 
                        tn_max = max(tn_ug), 
                        tn_sd = sd(tn_ug))

summary_tp <- summarise(processed_lake_nutrients, 
                        tp_mean = mean(tp_ug), 
                        tp_min = min(tp_ug), 
                        tp_max = max(tp_ug), 
                        tp_sd = sd(tp_ug))
...
kable(summary_tn, caption = "Summary Statistics Total Nitrogen")

kable(summary_tp, caption = "Summary Statistics Total Phosphorus" )
...
```{r plot 1, echo=FALSE, fig.align='left', fig.cap='Total Nitrogen by Lake Over Time'}
ggplot(processed_lake_nutrients, aes(x = sampledate, y = tn_ug, color = lakename))+
  geom_line()+
  labs(x = "Date", y = "Nitrogen (micrograms)", color = "Lake Name")
...
```{r plot 2, echo=FALSE, fig.align='left', fig.cap='Total Phosphorus by Lake Over Time'}

ggplot(processed_lake_nutrients, aes(x = sampledate, y = tp_ug, color = lakename))+
  geom_line()+
  labs(x = "Date", y = "Phosphorus (micrograms)", color = "Lake Name")
...
From the graphs created of phosphorus and nitrogen concentrations in a host of lakes in northern Wisconsin, we see a periodic spike in nutrient content that corresponds across several lakes and time periods. There is an especially large spike in concentrations of both nitrogen and phosphorus is about 1997. For both nutrients, most lakes reverted back towards the mean after this spike. 

***

For the next steps in analyzing the trends of nutrients in these lakes, I would investigate whether this trend is the result of natural processes or something like local agriculture in the area. To test this I would take a control lake that has no human inputs and a lake from the dataset above and run a comparison of means test to see if the test lake fell outside the probable difference in means nutrient levels between a pristine lake and a lake nearby human impacts.
