...
month2 <- c(1,7,2,8,3,9,4,10,5,11,6,12)
df_withdrawals <- data.frame("Month" = month2,
                             "Year" = rep(2000,12),
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant, #what does the double exclamation do?
...
ggplot(df_withdrawals,aes(x=Date,y=Avg_Withdrawals_mgd)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
  labs(title = paste("2015 Water usage data for",the_registrant),
       subtitle = the_facility_name,
       y="Withdrawal (mgd)",
       x="Date")
...
scrape.it <- function(the_year, the_facility){
  ...
  df_withdrawals <- data.frame("Month" = month2,
                             "Year" = rep(the_year,12),
                             "Avg_Withdrawals_mgd" = as.numeric(avg_withdrawals)) %>% #the_withdrawals vs avg_withdrawals
  arrange(Month) %>%
  mutate(Registrant = !!the_registrant,
         Facility_name = !!the_facility_name,
         Facility_type = !!the_facility_type,
...
ggplot(the_df,aes(x=Date,y=Avg_Withdrawals_mgd)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
  labs(title = paste("2018-2020 Water usage data for",the_registrant),
       subtitle = the_facility_name,
       y="Withdrawal (mgd)",
       x="Date")
```
