...
> There is strong evidence to suggest that the mean ozone level is below 50 (t = -57.98, df = 6829, p-value < 2.2e-16)
...
ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year)))+
  ...
  labs(title = "Ozone by Year", x = "Ozone AQI Value", y = "Density", fill = "Year")
#2
ggplot(EPAair, aes(x = Ozone, y = as.factor(Year)))+
  geom_boxplot()+
  labs(title = "Ozone By Year", y = "Year")+
  stat_summary(geom = "text", fun = max, hjust = -1, size = 5, label = c("a", "b"))
#3
ggplot(EPAair, aes(x = Ozone, y = as.factor(Year), color = as.factor(Year)))+
  geom_violin(draw_quantiles = c(0.5))+
  labs(title = "Ozone By Year", y = "Year")+
  stat_summary(geom = "text", fun = max, hjust = -1, size = 3.5, label = c("a", "b"))+
...
PM2.5_Ozone <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(PM2.5_Ozone)
h$r.squared
summary(PM2.5_Ozone)$coefficients[2,4]
...
EPAair_Cor_Data <- EPAair %>%
  select(meanLat:Ozone)%>%
  na.omit()

EPACorr <- cor(EPAair_Cor_Data)
...
EPAAIC <- lm(data = EPAair, PM2.5 ~ Ozone+Year+Month+meanLng+meanLat)
...
PM2.5_Ozone_New <- lm(data = EPAair, PM2.5 ~ Ozone + Month + meanLng + meanLat)
summary(PM2.5_Ozone_New)
