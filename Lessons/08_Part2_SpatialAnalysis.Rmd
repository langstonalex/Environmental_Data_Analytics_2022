...
huc8_sf2 <- st_read('./Data/Spatial/NCHUC8.shp') %>%
...
LabMap = mapview(huc8_sf,
                 col.regions = 'orange')
 

LabMap +  mapview(huc8_sf2,
                 col.regions = 'purple',
                 alpha = 1)
...
windData <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
                     colClasses = c('HUC_12' = 'factor')) %>%
  mutate(HUC_8 = substr(HUC_12, 1, 8)) %>%
  group_by(HUC_8) %>%
  dplyr :: summarize(windpower = sum(AvgWindEnergy))
